function replacer(replaces) {
    theReplacements = replaces
        .replace(/heck/gi, "H**k")
        .replace(/feck/gi, "F*ck")
        .replace(/darn/gi, "Heckn'")
        .replace(/Assassin/gi, "WooohooWoohooin")
        .replace(/Dude/gi, "Friendo");
...
for (let i = 0; i < listOfArrays.length; i++) {
    listOfArrays[i].forEach(replacer);
}
